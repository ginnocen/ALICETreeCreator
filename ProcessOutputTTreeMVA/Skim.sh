...
if [ -z "$4" ]; then
  stdoutputfile=$(printf "S_%s_stdout_%s-%s.txt" $trainname $datestamp2 $timestamp)
  stderrorfile=$(printf "S_%s_stderr_%s-%s.txt" $trainname $datestamp2 $timestamp)
else
  stdoutputfile=$(printf "S_%s_%s_stdout_%s-%s.txt" $trainname $stage $datestamp2 $timestamp)
  stderrorfile=$(printf "S_%s_%s_stderr_%s-%s.txt" $trainname $stage $datestamp2 $timestamp)
fi
