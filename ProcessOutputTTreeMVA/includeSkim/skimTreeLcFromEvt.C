...
void skimTreeLcFromEvt(TString input="AnalysisResults.root",TString output="test.root",TString ttreeout="tree_LctopKip", Bool_t isMC = kFALSE, Bool_t ispp = kFALSE){
...
/*-------------Output TTrees-------------*/
  ...
  TTree* fTreeLcGenML;
  if(isMC && tree_gen) fTreeLcGenML = new TTree("fTreeLcGenFlagged","fTreeLcGenFlagged");
  ...
  /*-------LcFlagged-------*/
  float inv_mass_ML, pt_cand_ML, d_len_ML, d_len_xy_ML, norm_dl_xy_ML, cos_p_ML, cos_p_xy_ML, imp_par_xy_ML, dca_ML, dist_12_ML, dist_23_ML, sig_vert_ML, max_norm_d0d0exp_ML;
  ...
  if(t.EnabledSkim_dist_12) fTreeLcML->Branch("dist_12_ML",&dist_12_ML,"dist_12_ML/F");
  if(t.EnabledSkim_dist_23) fTreeLcML->Branch("dist_23_ML",&dist_23_ML,"dist_23_ML/F");
  if(t.EnabledSkim_sig_vert) fTreeLcML->Branch("sig_vert_ML",&sig_vert_ML,"sig_vert_ML/F");
  if(t.EnabledSkim_max_norm_d0d0exp) fTreeLcML->Branch("max_norm_d0d0exp_ML",&max_norm_d0d0exp_ML,"max_norm_d0d0exp_ML/F");
  ...
  fTreeLcML->Branch("cand_fileID_ML",&cand_fileID_ML,"cand_fileID_ML/I");
  fTreeLcML->Branch("cand_evtID_ML",&cand_evtID_ML,"cand_evtID_ML/I");
  ...
  if(t.EnabledSkim_nsigTPC_K_0) fTreeLcML->Branch("nsigTPC_K_0_ML",&nsigTPC_K_0_ML,"nsigTPC_K_0_ML/F");
  ...
  if(t.EnabledSkim_nsigTOF_K_0) fTreeLcML->Branch("nsigTOF_K_0_ML",&nsigTOF_K_0_ML,"nsigTOF_K_0_ML/F");
  ...
  if(t.EnabledSkim_nsigTPC_K_1) fTreeLcML->Branch("nsigTPC_K_1_ML",&nsigTPC_K_1_ML,"nsigTPC_K_1_ML/F");
  ...
  if(t.EnabledSkim_nsigTOF_K_1) fTreeLcML->Branch("nsigTOF_K_1_ML",&nsigTOF_K_1_ML,"nsigTOF_K_1_ML/F");
  ...
  if(t.EnabledSkim_nsigTPC_K_2) fTreeLcML->Branch("nsigTPC_K_2_ML",&nsigTPC_K_2_ML,"nsigTPC_K_2_ML/F");
  ...
  if(t.EnabledSkim_nsigTOF_K_2) fTreeLcML->Branch("nsigTOF_K_2_ML",&nsigTOF_K_2_ML,"nsigTOF_K_2_ML/F");
  ...
  if(isMC && tree_gen){
    ...
    fTreeLcGenML->Branch("cand_fileID_gen_ML",&cand_fileID_gen_ML,"cand_fileID_gen_ML/I");
    fTreeLcGenML->Branch("cand_evtID_gen_ML",&cand_evtID_gen_ML,"cand_evtID_gen_ML/I");
  }
  ...
  for(Long64_t jentry=0; jentry<nevt;jentry++){
    ...
    for(int icand = 0; icand < t.n_cand; icand++){
      ...
      if(t.dist_12) dist_12_ML=t.dist_12 -> at(icand);
      if(t.dist_23) dist_23_ML=t.dist_23 -> at(icand);
...
int main(int argc, char *argv[])
{
  ...
  if(argc == 6)
    skimTreeLcFromEvt(argv[1],argv[2],argv[3],atoi(argv[4]),atoi(argv[5]));
  return 0;
}
