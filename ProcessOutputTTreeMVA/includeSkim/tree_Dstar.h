...
// from TTree tree_Dstar/tree_Dstar
...
#ifdef tree_Dstar_cxx
tree_Dstar::tree_Dstar(TTree *tree) : fChain(0)
{
...
//   if (tree == 0) {
      ...
      // }
      //f->GetObject("tree_Dstar",tree);

      //}
   Init(tree);
}

tree_Dstar::~tree_Dstar()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t tree_Dstar::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Int_t tree_Dstar::GetEntriesFast()
{
  // Read contents of entry.
  if (!fChain) return 0;
  return fChain->GetEntriesFast();
}
Long64_t tree_Dstar::LoadTree(Long64_t entry)
{
...
void tree_Dstar::Init(TTree *tree)
{
...
void tree_Dstar::Show(Long64_t entry)
{
...
Int_t tree_Dstar::Cut(Long64_t entry)
{
