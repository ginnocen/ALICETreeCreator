...
doDplus=1       #toset (mergers)
doDs=0          #toset (mergers)
doDzero=0       #toset (mergers)
doDstar=0       #toset (mergers)
doLcpKpi=0      #toset (mergers)
doLcpK0s=0      #toset (mergers)
...
if [ -z "$4" ]; then
  stdoutputfile=$(printf "M_%s_stdout_%s-%s.txt" $trainname $datestamp2 $timestamp)
  stderrorfile=$(printf "M_%s_stderr_%s-%s.txt" $trainname $datestamp2 $timestamp)
else
  stdoutputfile=$(printf "M_%s_%s_stdout_%s-%s.txt" $trainname $stage $datestamp2 $timestamp)
  stderrorfile=$(printf "M_%s_%s_stderr_%s-%s.txt" $trainname $stage $datestamp2 $timestamp)
fi
