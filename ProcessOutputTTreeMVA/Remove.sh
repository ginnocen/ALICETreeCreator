...
if [ -z "$4" ]; then
  stdoutputfile=$(printf "R_%s_stdout_%s-%s.txt" $trainname $datestamp2 $timestamp)
  stderrorfile=$(printf "R_%s_stderr_%s-%s.txt" $trainname $datestamp2 $timestamp)
else
  stdoutputfile=$(printf "R_%s_%s_stdout_%s-%s.txt" $trainname $stage $datestamp2 $timestamp)
  stderrorfile=$(printf "R_%s_%s_stderr_%s-%s.txt" $trainname $stage $datestamp2 $timestamp)
fi
...
do
  ...
  if [ "$foundchild" == "1" ]; then
    ...
    sh ./run_remover $runremover $outputlist $isMC $doDplus $doDs $doDzero $doDstar $doLcpKpi $doLcpK0s >> "$removeroutputfile" 2>> "$removererrorfile"
    ...
    cat "$removeroutputfile" >> "$stdoutputfile"
    cat "$removererrorfile" >> "$stderrorfile"
    rm "$removeroutputfile" "$removererrorfile"
  fi
