...
//_________________________________________________________________________________________________________
//prototypes
void RunAnalysisAODVertexingHFDplus_TreeMVA_ROOT5(TString pluginmode = "full", Bool_t fIsMC=kFALSE, TString period = "LHC17q"/*"LHC18a4a2_fast"*/, TString recopass = "pass1_CENT_wSDD"/*""*/);
...
//_________________________________________________________________________________________________________
//impelementation
void RunAnalysisAODVertexingHFDplus_TreeMVA_ROOT5(TString pluginmode, Bool_t fIsMC, TString period, TString recopass) {
  ...
  gROOT->LoadMacro("AliAnalysisTaskSEDDplus_TreeMVA.cxx+g");
  gROOT->LoadMacro("AddTaskDplus_TreeMVA.C");
  TString cutfilename_consPID="alien:///alice/cern.ch/user/f/fgrosa/DplustoKpipiCuts_pp_loose_consPID_kINT7.root";
  TString suffix_consPID_MVA="_MB_Loose_consPID_MVA_kINT7";
  Int_t sparseopt=0; //enables THnSparses
  Int_t treeopt=1; //enables TTree
  AliAnalysisTaskSEDplus_TreeMVA* taskDplus_consPID_MVA = AddTaskDplus_TreeMVA(0,0,100,treeopt,sparseopt,kFALSE,fIsMC,suffix_consPID_MVA.Data(),cutfilename_consPID.Data());
...
AliAnalysisGrid* CreateAlienHandler(TString pluginmode, TString testfileslistWithPlugin, Bool_t fIsMC, TString period, TString recopass) {
  ...
  // One can always follow the standard procedure of calling alien-token-init then
   ...
   plugin->SetAliPhysicsVersion("vAN-20181003-1");
  ...
  TString friendchainname = "./AliAOD.VertexingHF.root";
   ...
   plugin->SetGridWorkingDir(Form("Dplus_pp_5TeV_rawPID_%s_%s",period.Data(),recopass.Data()));
   // Name of executable
   plugin->SetExecutable(Form("Dplus_pp_5TeV_rawPID_%s_%s.sh",period.Data(),recopass.Data()));
   // Declare alien output directory. Relative to working directory.
   plugin->SetGridOutputDir("output");
   // Declare the analysis source files names separated by blancs. To be compiled runtime
   gROOT->ProcessLine(".L AliHFTreeHandler.cxx++g");
   gROOT->ProcessLine(".L AliHFTreeHandlerDplustoKpipi.cxx++g");
   gROOT->ProcessLine(".L AliAnalysisTaskSEDplus_TreeMVA.cxx+g");
   plugin->SetAnalysisSource("AliHFTreeHandler.cxx AliHFTreeHandlerDplustoKpipi.cxx AliAnalysisTaskSEDplus_TreeMVA.cxx");

   // Declare all libraries (other than the default ones for the framework. These will be
   // loaded by the generated analysis macro. Add all extra files (task .cxx/.h) here.
   plugin->SetAdditionalLibs("libPWGflowBase.so libPWGflowTasks.so libPWGHFbase.so libPWGHFvertexingHF.so AliHFTreeHandler.h AliHFTreeHandler.cxx AliHFTreeHandlerDplustoKpipi.h AliHFTreeHandlerDplustoKpipi.cxx AliAnalysisTaskSEDplus_TreeMVA.h AliAnalysisTaskSEDplus_TreeMVA.cxx");
