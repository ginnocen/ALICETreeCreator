...
AliAnalysisTaskSEBPlustoD0Pi_TreeMVA *AddTaskBPlustoD0Pi_TreeMVA(
                                TString cutsfile="",
                                ...
                                Int_t system=0,
                                Int_t nWriteVariableTree=2,
                                Bool_t bWriteOnlySignalTree=kFALSE
                                )
{
  ...
  // gDebug = 1;
  if (!mgr) {
    ::Error("AddTaskBPlustoD0Pi_TreeMVA", "No analysis manager to connect to.");
    return NULL;
  }  

  if(nUseQuickSignalAnalysis != 0 && !bUseMCInfo){
    ::Error("AddTaskBPlustoD0Pi_TreeMVA", "Quick signal analysis only works for Monte Carlo.");
    return NULL;
  }  
  ...
  // create the task
  AliAnalysisTaskSEBPlustoD0Pi_TreeMVA *task = new AliAnalysisTaskSEBPlustoD0Pi_TreeMVA("AliAnalysisTaskSEBPlustoD0Pi_TreeMVA",RDHFBPlustoD0Pi);
  ...
  task->SetWriteVariableTree(nWriteVariableTree);
  task->SetWriteOnlySignalTree(bWriteOnlySignalTree);
  mgr->AddTask(task);
 ...
 // ----- output data -----
  ...
  AliAnalysisDataContainer *coutputBPlusTree =                 mgr->CreateContainer(output10,TTree::Class(),AliAnalysisManager::kOutputContainer, outputfile.Data());
  ...
  mgr->ConnectOutput(task,10,coutputBPlusTree);
