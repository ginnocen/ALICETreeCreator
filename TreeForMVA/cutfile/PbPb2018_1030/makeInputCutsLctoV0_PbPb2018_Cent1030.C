...
AliRDHFCutsLctoV0 *makeInputCutsLctoV0(Int_t whichCuts=0, TString nameCuts="LctoV0FilteringCuts", Float_t minc=0., Float_t maxc=10., Bool_t isMC=kFALSE, Int_t OptPreSelect = 1, Int_t TPCClsPID = 50, Bool_t PIDcorrection=kTRUE, Double_t minpt = 4, Bool_t spdkAny=kTRUE, Bool_t ispass1 = kFALSE)
{
  ...
  //NB: No cut is placed for SetTrackFilterBachelor, so doesn't matter
  ...
  if(whichCuts==0){
    ...
    Float_t cuts[nptbins][nvars]={
      0.2,0.,0.03,0.05,0.5,0.0,0.0,10.,2.,0.998,1.,3.,0.,0.,0.,0.7,9999.,-9999.,-9999.,-9999.,0.,1000.,1,
      0.2,0.,0.03,0.05,0.5,0.0,0.0,10.,2.,0.999,1.,3.,0.,0.,0.,0.7,9999.,-9999.,-9999.,-9999.,0.,1000.,1
    };
  ...
  if(!isMC && PIDcorrection) cutsLctoV0->EnableNsigmaDataDrivenCorrection(kTRUE, AliAODPidHF::kPbPb1030);
  
  //event selection
  cutsLctoV0->SetUsePhysicsSelection(kTRUE);
  cutsLctoV0->SetTriggerClass("");
  if(!isMC) cutsLctoV0->SetTriggerMask(AliVEvent::kINT7);
