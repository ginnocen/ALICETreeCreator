void debug_(std::string filename="AnalysisResults.root", int index=0, Long64_t nev = TTree::kMaxEntries)
{
  ...
  TFile *fout = new TFile(TString::Format("histo%d.root", index), "recreate");
  fout->cd();
  h1->Write();
  h2->Write();
  hntrk_int7->Write();
  hntrk_int7_withd->Write();
}
